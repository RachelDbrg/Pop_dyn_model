...
caribou_nodeath <- caribou_nodeath %>% 
  pull(outputs) %>%   
  map_dfr(as.data.frame) %>% 
  mutate_all(as.numeric) %>% 
  mutate(zone = "C")

noprey_nodeath <- noprey_nodeath %>% 
  pull(outputs) %>%   
  map_dfr(as.data.frame) %>% 
  mutate_all(as.numeric) %>% 
  mutate(zone = "Q")

moose_nodeath <- moose_nodeath %>% 
  pull(outputs) %>%   
  map_dfr(as.data.frame) %>% 
  mutate_all(as.numeric) %>% 
  mutate(zone = "W")

lichen_caribou_moose_deer_wolf <- lichen_caribou_moose_deer_wolf %>% 
  pull(outputs) %>%   
  map_dfr(as.data.frame) %>% 
  mutate_all(as.numeric) %>% 
  mutate(zone = "lichen_caribou_moose_deer_wolf")


lichen_caribou_moose_deer_additional_prey_wolf <- lichen_caribou_moose_deer_additional_prey_wolf %>% 
  pull(outputs) %>%   
  map_dfr(as.data.frame) %>% 
  mutate_all(as.numeric) %>% 
  mutate(zone = "lichen_caribou_moose_deer_additional_prey_wolf")


carrying_capacities <- lichen_caribou_moose_wolf |> 
  filter(time == 300) |> 
...
test <- rbind(noprey_nodeath, moose_nodeath)
test <- rbind(test, caribou_nodeath)
test <- rbind(test, lichen_caribou_moose_deer_additional_prey_wolf)


moose_nodeath %>%
  # filter(PP == 1) %>%
  pivot_longer(cols = c(Na, V, Pa, Ma, Ca, Qa, k_P),
               names_to = "species",
               values_to = "density") %>%
  mutate(species = case_when(
    species == "V"  ~ "lichen",
